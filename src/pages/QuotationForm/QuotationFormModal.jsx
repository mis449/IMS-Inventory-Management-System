...
export default function QuotationFormModal({ isOpen, onClose, onSave, initialData, onConvertToInvoice, onDelete, onCopy }) {
  ...
  return (
    ...
    <ModalForm
      isOpen={isOpen}
      onClose={onClose}
      title={initialData ? "Edit Quotation" : "Quotation Form"}
        ...
        <div className="min-h-[250px] py-4">
          ...
          {activeTab === 'OtherInfo' && (
            <OtherInformationTab 
              ...
              quotationStatus={quotationStatus} setQuotationStatus={setQuotationStatus}
          ...
          {activeTab === 'Notes' && (
            <div className="space-y-5 px-2">
               <div className="space-y-1.5">
                  ...
                  <textarea rows="2" value={notes.remarks} onChange={(e) => setNotes({...notes, remarks: e.target.value})} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-sky-500/10 focus:border-sky-500 text-xs md:text-sm bg-white outline-none"></textarea>
               </div>
               <div className="space-y-1.5">
                  <label className="block text-[11px] text-slate-700 font-bold uppercase tracking-wider">Terms & Conditions</label>
                  <textarea rows="3" value={notes.termsAndConditions} onChange={(e) => setNotes({...notes, termsAndConditions: e.target.value})} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-sky-500/10 focus:border-sky-500 text-xs md:text-sm bg-white outline-none"></textarea>
               </div>
               <div className="space-y-1.5">
                  <label className="block text-[11px] text-slate-700 font-bold uppercase tracking-wider">Additional Notes</label>
                  <textarea rows="2" value={notes.additionalNotes} onChange={(e) => setNotes({...notes, additionalNotes: e.target.value})} className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-sky-500/10 focus:border-sky-500 text-xs md:text-sm bg-white outline-none"></textarea>
               </div>
            </div>
          )}
        ...
        <div className={`w-full mx-auto flex flex-col flex-1 min-h-0 mt-10 shadow-2xl rounded-2xl ${
          ...
          <div className="flex-1 w-full overflow-y-auto min-h-0 rounded-b-2xl">
            ...
            <div className="flex justify-between items-start border-b-2 border-sky-600 pb-5 mb-6">
              <div>
                <h1 className="text-xl font-black text-sky-800 uppercase tracking-wider">Parekh Gallerium</h1>
              ...
              <div className="text-right">
                <h2 className="text-3xl font-black text-slate-900 tracking-tight">QUOTATION</h2>
                <div className="mt-2 text-xs text-slate-500 space-y-1">
                  <p><span className="font-semibold text-slate-700">Quotation No:</span> {initialData?.quotationNo || 'Draft'}</p>
                  ...
                  <p><span className="font-semibold text-slate-700">Status:</span> <span className="uppercase font-bold text-sky-600">{quotationStatus === 'Final' ? 'Completed' : quotationStatus}</span></p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-5 gap-6 text-xs mb-10">
              <div className="col-span-3 space-y-4">
                {notes.remarks && (
                  <div>
                    <h4 className="font-bold text-slate-850 uppercase tracking-wider text-[9px]">Remarks</h4>
                    <p className="text-slate-500 whitespace-pre-line mt-1 p-2 bg-slate-50 rounded-lg border border-slate-100">{notes.remarks}</p>
                  </div>
                )}
                {(notes.termsAndConditions || notes.additionalNotes) && (
                  <div>
                    <h4 className="font-bold text-slate-850 uppercase tracking-wider text-[9px]">Terms & Conditions</h4>
                    <p className="text-slate-500 whitespace-pre-line mt-1 p-2 bg-slate-50 rounded-lg border border-slate-100">{notes.termsAndConditions || notes.additionalNotes}</p>
                  </div>
                )}
              </div>
